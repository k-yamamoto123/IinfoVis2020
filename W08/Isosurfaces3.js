function Isosurfaces( volume, isovalue )
{
    ...
    function interpolated_vertex( v0, v1, s )
    {
        var lines = volume.resolution.x;
        var slices = volume.resolution.x * volume.resolution.y;
        var v0id = v0.x + v0.y * lines + v0.z * slices;
        var v1id = v1.x + v1.y * lines + v1.z * slices;
        var s0 = volume.values[ v0id ][0];
        var s1 = volume.values[ v1id ][0];
        var t = (s - s0)/(s1 - s0);
        return new THREE.Vector3().addVectors( new THREE.Vector3(v0.x * (1 - t) , v0.y * (1-t), v0.z * (1 - t)) , new THREE.Vector3(v1.x * t , v1.y * t, v1.z * t) );
    }
}
