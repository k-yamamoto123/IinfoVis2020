function Isosurfaces( volume, isovalue )
{
    ...
    for ( var i = 0; i < 256; i++ )
    {
        ...
        var G = Math.max( Math.cos( ( S - 0.5 ) * Math.PI ), 0.0 );
        var B = Math.max( Math.cos( (S - 1.0  )* Math.PI ), 0.0 );
        var color = new THREE.Color( R, G, B );
        cmap3.push( [ S, '0x' + color.getHexString() ] );
    }

    switch(cmap_iso){
        case 1:    material.color = new THREE.Color().setHex( cmap1[ isovalue][1] );
        break;
        case 2: material.color = new THREE.Color().setHex( cmap2[ isovalue][1] );
        break;
        case 3: material.color = new THREE.Color().setHex( cmap3[ isovalue][1] );
        break;   
    }
